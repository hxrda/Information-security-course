...
<ins>Password manager choice: KeePassXC</ins>  

- KeePassXC is a open-source, cross-platform, offline/cloud-free and feature rich password manager. It stores passwords and other types of information (usernames, URLs, attachments etc.) in a local encrypted database. When accessing the database (via KeePass), the stored data is decrypted and stored in the local memory of the device. 

<ins>Install the password manager</ins> 

- Note: the installation will be done on a Linux Debian system 
- `sudo apt-get update && sudo apt-get upgrade`
- `sudo apt install keepassxc`


<ins>Launch the password manager</ins> 

- `keepassxc ` (from terminal)
  
<ins>How to use KeePassXC</ins>

1. Create a database
- The db will store your passwords and other details
- Launch the application and click on `Create new database` 

![ssh](h5-images/d_11.png)

- Proceed with the prompts for naming the database, encryption settings, giving database credentials and selecting the location of the database file (.kbdx). A strong password is recommended. 

- (Alternatively, if a db has been created previously, click `Open existing database)` and provide the db password)

2. Add & store a password entry
- Start adding passwords into the database by either:
	- clicking `Add new entry`
	- navigating to `Entries -> New entry`
	- pressing `Ctrl+N`

- Enter a desired title for the entry, username, password, URL, and notes
	- A secure random password can be generated by clicking the dice icon in the password field

- Save the entry by pressing `OK`
  
![ssh](h5-images/d_2.png)

![ssh](h5-images/d_3.png)
...
- `Database -> Save Database Backup…` or automated backups from the application settings


<ins>Necessity of password managers</ins>  

- Password managers improve protection against the following security risks:  
	- **Password reuse & credential stuffing**: Instead of using a single password for multiple accounts, password managers make it convenient to generate and use unique passwords for each service. As a consequence, if one account gets compromised, the attacker will not gain access to several other accounts as well.
	- **Weak, easy-to-crack passwords & brute force attacks**: While users tend to come up with simplistic passwords for convenience and memorization purposes, password managers can be used to generate strong and randomized passwords with a few clicks. This helps e.g. protect against brute force attempts at cracking passwords. 
	- **Phishing attacks & keylogging**: Some passwords managers offer an autofill feature which populates only fields on domains that are marked as trusted (when correct settings are in place). The password manager will examine the domain’s legitimacy and verify no phishing elements are present. The risk of keyloggers capturing keystrokes is also reduced.
	- **Unauthorized access & data loss**: Storing passwords in an encrypted location help protect the data even if the device is lost or stolen, as long as the master key remains a secret and proper configurations are in place (e.g. autofill feature settings). 

- Other benefits of password managers:  
	- **Encryption & secure storage**: The password database is encrypted. E.g. the KeePassXC’s databases are protected with AES256 encryption, one of the mos secure encryption standards. Additionally, optional configurations are available for strengthening master passwords.
	- **Offline tools**: Some password managers operate completely offline, protecting against potential data leaks that could happen if using third party/cloud services. 
	- **Automatic updates**: Password managers can be used to periodically and proactively update passwords. Expiration dates and notifications can be sept to prompt updates. 


<ins>References</ins> 
- KeePassXC.(2025). Documentation and FAQ - KeePassXC. Available at: https://keepassxc.org/docs/.
- KeePassXC Team. (2022). KeePassXC: Getting Started Guide. Keepassxc.org. Available at: https://keepassxc.org/docs/KeePassXC_GettingStarted
- KeePassXC Team. (2024). KeePassXC: User Guide. [online] Keepassxc.org. Available at: https://keepassxc.org/docs/KeePassXC_UserGuide.
- mSecure. (2024). mSecure • The Benefits of Using a Password Manager in 2024. Available at: https://www.msecure.com/blog/the-benefits-of-using-a-password-manager-in-2024/ 
- Menzies, K. (2024). Is it safe to let your password manager autofill your password? Proton. Available at: https://proton.me/blog/safe-to-autofill-passwords
